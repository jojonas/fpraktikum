...
class Fit:
	...
	def plot_residual(self, *, box=False, logx=False, logy=False, **plotopts):
	...
	def plot(self, range=None, N=100, *, plot_data=True, plot_fit=True, box=False, logx=False, logy=False, units={}, factors={}, **plotopts):
		...
		if plot_data and self._x is not None and self._y is not None:
			...
			for plotopt, value in plotopts.items():
				if plotopt.startswith("data_"):
					plotopt = plotopt[len("data_"):]
				if not plotopt.startswith("fit_"):
					options[plotopt] = value
		...
		if plot_fit:
			...
			for plotopt, value in plotopts.items():
				if plotopt.startswith("fit_"):
					plotopt = plotopt[len("fit_"):]
				if not plotopt.startswith("data_"):
					options[plotopt] = value
